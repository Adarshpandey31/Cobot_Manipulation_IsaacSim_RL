...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # Keep only algorithm keys accepted by installed PPO,
    # but preserve "class_name" because OnPolicyRunner requires it.
    if "algorithm" in agent_cfg_dict:
        original_algo_cfg = agent_cfg_dict["algorithm"]

        # Preserve runner-required algorithm class name
        ...
        filtered_algo_cfg.update(
            {k: v for k, v in original_algo_cfg.items() if k in valid_algo_keys}
        )

        removed_algo_keys = sorted(
            set(original_algo_cfg.keys()) - set(filtered_algo_cfg.keys())
        )
        if removed_algo_keys:
            print("[INFO] Removed unsupported PPO algorithm keys:", removed_algo_keys)

        print("[INFO] Final PPO algorithm keys:", sorted(filtered_algo_cfg.keys()))
